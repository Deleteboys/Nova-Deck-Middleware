...
impl Action for MasterVolumeAction {
    fn execute(&self) {
        ...
        tauri::async_runtime::spawn(async move {
            let Ok(current_vol) = get_master_volume() else {
                debug!("Konnte aktuelle System-Lautstärke nicht auslesen.");
                return;
            };
            ...
            if let Err(e) = set_master_volume(new_vol) {
                error!("Fehler beim Setzen der System-Lautstärke: {}", e);
            } else {
                debug!(
                    "System-Lautstärke von {:.0}% auf {:.0}% gesetzt",
                    current_vol * 100.0,
                    new_vol * 100.0
                );
            }
